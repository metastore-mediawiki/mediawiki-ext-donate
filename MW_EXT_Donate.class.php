...
class MW_EXT_Donate {
	...
	public static function onRenderTag( Parser $parser, PPFrame $frame, $args = [] ) {
		...
		// Argument: receiver.
		$getWallet = MW_EXT_Core::outClear( $getOption['ya-wallet'] ?? '' ?: '' );
		$outWallet = $getWallet;

		// Argument: fio.
		$getFIO = MW_EXT_Core::outClear( $getOption['ya-fio'] ?? '' ?: '' );
		$outFIO = $getFIO;

		// Argument: email.
		$getEmail = MW_EXT_Core::outClear( $getOption['ya-email'] ?? '' ?: '' );
		$outEmail = $getEmail;

		// Argument: phone.
		$getPhone = MW_EXT_Core::outClear( $getOption['ya-phone'] ?? '' ?: '' );
		$outPhone = $getPhone;

		// Argument: address.
		$getAddress = MW_EXT_Core::outClear( $getOption['ya-address'] ?? '' ?: '' );
		$outAddress = $getAddress;

		// Argument: target.
		$getTargets = MW_EXT_Core::outClear( $getOption['ya-target'] ?? '' ?: MW_EXT_Core::getMessageText( 'donate', 'targets' ) );
		$outTargets = $getTargets;

		// Argument: sum.
		$getSum = MW_EXT_Core::outClear( $getOption['ya-sum'] ?? '' ?: '100' );
		$outSum = $getSum;
		...
		// Argument: card.
		$getCard = MW_EXT_Core::outClear( $getOption['card'] ?? '' ?: '' );
		$outCard = $getCard;

		// Argument: paypal.
		$getPayPal = MW_EXT_Core::outClear( $getOption['paypal'] ?? '' ?: '' );
		$outPayPal = $getPayPal;

		// Argument: bitcoin.
		$getBitcoin = MW_EXT_Core::outClear( $getOption['bitcoin'] ?? '' ?: '' );
		$outBitcoin = $getBitcoin;

		// Argument: ethereum.
		$getEthereum = MW_EXT_Core::outClear( $getOption['ethereum'] ?? '' ?: '' );
		$outEthereum = $getEthereum;

		// Argument: monero.
		$getMonero = MW_EXT_Core::outClear( $getOption['monero'] ?? '' ?: '' );
		$outMonero = $getMonero;
		...
		if ( $outWallet ) {
			...
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-yandex' ) . '</div>';
			...
			$outHTML .= '<div><div><input name="sum" value="' . $outSum . '" data-type="number" type="number" placeholder="' . MW_EXT_Core::getMessageText( 'donate', 'sum-placeholder' ) . '" /></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-yandex-desc-sum' ) . '</div></div>';
			$outHTML .= '<div><div><textarea name="comment" placeholder="' . MW_EXT_Core::getMessageText( 'donate', 'comment-placeholder' ) . '"></textarea></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-yandex-desc-comment' ) . '</div></div>';
			...
			$outHTML .= '</div><div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-yandex-desc-payment-type' ) . '</div>';
			$outHTML .= '</div><div class="mw-ext-donate-submit">';
			$outHTML .= '<input class="mw-ext-donate-button" value="' . MW_EXT_Core::getMessageText( 'donate', 'submit' ) . '" type="submit" />';
		...
		if ( $outCard ) {
			...
			$outHTML .= '<div class="mw-ext-donate-content">';
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-card' ) . '</div>';
			$outHTML .= '<div><input value="' . $outCard . '" type="text" readonly /></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-card-desc' ) . '</div>';
			$outHTML .= '</div></div>';
		}
		...
		if ( $outPayPal ) {
			...
			$outHTML .= '<div class="mw-ext-donate-content">';
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-paypal' ) . '</div>';
			$outHTML .= '<div><a class="mw-ext-donate-button" href="https://www.paypal.me/' . $outPayPal . '" rel="nofollow" target="_blank">' . $outPayPal . '</a></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-paypal-desc' ) . '</div>';
			$outHTML .= '</div></div>';
		}
		...
		if ( $outBitcoin ) {
			...
			$outHTML .= '<div class="mw-ext-donate-content">';
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-bitcoin' ) . '</div>';
			$outHTML .= '<div><input value="' . $outBitcoin . '" type="text" readonly /></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-bitcoin-desc' ) . '</div>';
			$outHTML .= '</div></div>';
		}
		...
		if ( $outEthereum ) {
			...
			$outHTML .= '<div class="mw-ext-donate-content">';
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-ethereum' ) . '</div>';
			$outHTML .= '<div><input value="' . $outEthereum . '" type="text" readonly /></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-ethereum-desc' ) . '</div>';
			$outHTML .= '</div></div>';
		}
		...
		if ( $outMonero ) {
			...
			$outHTML .= '<div class="mw-ext-donate-content">';
			$outHTML .= '<div class="mw-ext-donate-title">' . MW_EXT_Core::getMessageText( 'donate', 'to-monero' ) . '</div>';
			$outHTML .= '<div><input value="' . $outMonero . '" type="text" readonly /></div>';
			$outHTML .= '<div class="mw-ext-donate-desc">' . MW_EXT_Core::getMessageText( 'donate', 'to-monero-desc' ) . '</div>';
			$outHTML .= '</div></div>';
		}
